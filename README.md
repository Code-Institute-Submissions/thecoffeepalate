...
Whilst trying to generate the business logo through [Tailor Brand](https://studio.tailorbrands.com/) I came across several appealing color schemes. Afterwards, I have transferred the most appealing one to [Coolors](https://coolors.co/) where, 
through slight adjustments, came up with a final, strong and harmonious scheme.
...
For instance, the navigation bar will collapse into a hamburger icon in mobile devices.
...
The metallic-seaweed color will be and "aid" color. It will be used, if necessary, to separate sections or highlight borders.
...
* **Navigation:**
As you can see on my wireframes, the original idea was to have a home button in the navbar. Nonetheless, this option was a little redundant as the logo and brand had the same effect throughout all the pages.
Therefore, the next step was to put aside the home button and substitute it for a subscribe button instead linking it to the subscribe section (See next bullet point).

* **Subscription section:**
The susbcription section is also something that was not forecasted during the website design, but is an important tool to bring a better user experience. Therefore, adding this to the website was a logic step
to take.

Firstly, it was implemented in the footer, but after discussing it with my mentor, the solution passed by creating a separate section for this option.

The modal that pops-up when clicking subscribe was also not forecasted, but certainly it gives a great interaction with the user.

* **Tabs vs Carousels:**
Whilst designing the website, the current carousels applied to _The Lab_ page were firstly idealized as tabs.

After further review and research, the carousel option started to be more appealing due to its easier implementation, responsiveness and overall attractivness. This resulted in some trade-off 
regarding the content to be displayed, but without affecting the outcome for the user.

Overall, the final result looks much better than the idealized.

* **Order Form:**

When designing the wireframes, I did not notice that the pricing content was not putted in place. 

This generated a more complex aproach in order to maintain the max. 4 steps per order process. To do this, I have merged the pricing and amount content into to the _grind selection_, righ before the _contact details_.

The order form initially was designed not to be in a carousel but placed throughout the page vertically. 

Nonetheless, despite of the increased complexity of embedding a form into a carousel, I decided to endeavor in this additional challenge in order to improve the UX, maintaining the same structure as the other two pages.

* **Icons:**
I did not apply the icon idealized for the hero-container in _The Store_ has it would generate too much visual noise.

The representative icons used in the form in the wireframes were later susbstituted for batteries icons, where the charge of the battery increases every step of the way in the form until is fully charged.


* **Large screens layout:**
Initially, the _Landing page_ was designed to include only 1 hero-image and two smaller images dividing the portview into 50/50. This option did not end up to be too appealing, the reason why I have switched for 3 hero-containers.

This ended up being a win for the project.
...
### **Existing Features**
#### ***Navigation bar***
For the navigation bar, I have used Bootstrap to implement a responsive navbar across devices. In this case, the navbar is 100% visible in large screens, while on small/medium devices it will collapse into a dropdown menu, resulting in space saving of the real-estate.
...
On the following two sections, custom buttons were implemented to give the user another possibility than the navbar to navigate to the corresponding pages.

The last section is the subscription section, which is implemented the same way across the pages.

#### ***The Lab***
The Lab page has been built following closely the design structure of the landing page. 

The introductory section uses a hero-image, whereas the following two sections have implemented a carousel each. The page body ends at the subscription section.
...
On the same line as [The-Lab], this page also run into the same issues with the carousel. Instead of having variable content across slides (i.e text and graphs/diagrams) a form has been implemented.

Embedding a form that has several steps, and consequently varies in height throughout the form itself, into a carousel was quite tricky.
...
#### ***Subscription***
The subscription section was an idea that came across through the actual coding and not idealized in the design process, result of missing some more interactivity with the user.
...
#### ***Footer/Contacts***
The contact options are placed in the footer of the page, displaying social links and e-mail options, via [FontAwesome](https://fontawesome.com/ "FontAwesome") for the user to get in contact with the company.

This is a simple and intuitive way of approaching it.

#### ***Modals***
Feedback to the user throughout the website and actions is fundamental. To address this user need, I have decided to implement modals linked to the "Subscribe" and "Place Order" buttons. 
...
* Validation system to guarantee that user can only proceed from first slide on _The Store_ form after the blend totals 100%.
* Packaging customization form, where user uploads his/her own brand/logo to be applied on their coffee bags
* Immediate payment of the order through credit card, [Stripe](https://stripe.com/en-nl "Stripe") and [Paypal](https://www.paypal.com/nl/home "Paypal")
...
* Implement an interactive shopping cart so the user can visualize in real-time his/her order and correspondent pricing
...
After accepting *Terms&Conditions*, the user receives immediate feedback provided with a modal that the order has been placed and which steps follow.

##### **Testing** #####

I have gone through the four steps of the order and found out that the pricing method displayed is not sufficient clear.
Also, as we are in a middle of a form and don't want to lose the data inserted so far, by clicking on the link back to *The Lab*, this was opening in the same tab which was afecting the user experience. Thereof, target="_blank" attribute has been included in the corresponding anchor element.

##### **Test result** #####

The order testing clearly passes 4 out of 6 stories and requirements. 
The first topic to address in future releases would be the tooltips, that due to time constraints I was not able to implement in due course. Using Bootstrap tooltips might be the fastest option to guarantee compatibility and responsiveness with the rest of the website.
The second and final topic to tackle in future releases would be the clear pricing information, for which I would have to use deeper developing languages, like Javascript, and at the moment I don't have the knowledge to implement an interactive shopping cart so the user can visualize in real-time his/her order and correspondent pricing.
...
Has both carousels present only 5 items each, using this option is appropriate. If there were a lot more items, most probbably I would have to re-think the full content display on the page.
...
Moreover, whilst the first carousel presents fun facts about the origin to make the content more appealing, the second carousel presents *tips* instead. This because the choosing process is more complex during this stage than choosing the favourite origin based on the radial graphs presented.
...
##### **Test result** #####
Carousels are working properly and all the info is passed without any issue to the user.
...
Moreover, on big screens the user can know on each section of the page he/she is due to the highlight of the specific section.
...
At this stage, user is faced with 2 carousels displaying useful information for the blending process. These present a very easy, intuitive and appealing way to navigate through the info.
...
* Carousels
The carousels implemented to show the content and the form, together with d-flex and h-100 bootstrap classes guarantees that they are fully responsive across devices.

* Forms
To embed the form in the carousel, I have opened it right at the beginning of the carousel and closed right before the end. The formatting was made with Bootstrap guaranteeing its responsiveness.
...
##### **Testing** #####
To test the navigation, I have conducted a thorough analysis going page by page and clicking on any button available to guarantee that there is nothing not functioning properly.
Started with the navbar in small-devices up to big-screens, analyzed in-between pages buttons and carousel and modals buttons, finishing with the anchor elements at the footer of the pages.

##### **Test result** #####
Website does not show any contradictory information, has a well-defined structure and is easy to navigate.

Colors and images bring a perfect identity to every step/location where the user is navigating. 

External links are internal *The Lab* links within the carousel form, open in a new tab to guarantee that the user does not lose track of what he/she is doing, nor information already inputted in the form.
...
##### **Testing** #####
To test this, I have ran a thorough inspection of the footer element, clicking link by link to guarantee its workability as desired.
...
##### **Bug** #####
Dropdown menu not collapsing automatically when choosing the subscribe option.
##### **(Potential) Corrections** #####
...
##### **Bug** #####
User can proceed through the first and third step of the order form without selecting the percentages of coffee for the blend and without selecting the amount of bags/amount of coffee per bag.
##### **(Potential) Corrections** #####
In the future I would like implement some validation code on this steps to prevent the user of proceeding forward without filling this content.
For instance, if the total of the blend does not equal 100%, the user will not be able to go to step 2.
...
To minimize the impact of this issue, I have chosen to place clear warnings/advice throughout the form.
...
* 3 - Choose the correspondent master branch on the GitHub Pages section and click save
...
*Side note*: Graphs and diagrams are own-creation.
...
Also, some parts of the code have been based on the content available in [W3Schools](https://www.w3schools.com/ "W3Schools") and [Bootstrap](https://getbootstrap.com/ "Bootstrap") that allowed me to attain the final result.
...
Last but for sure not the least, a special thanks to my wife-to-be, [Anouk](https://github.com/AnoukSmet/ "Anouk Smet Github page"), for all the suggestions, inputs, testing and above all for the extreme patience and support shown during this journey.
